...
interface S2ShapeAspect {
  ...
  interface ChainAspect {
    ...
    /** Returns vertices of each chain in this shape. Less efficient but may be more convenient. */
    ...
    abstract class Simple implements Mixed {
      ...
      @Override public List<List<S2Point>> chains() {
    ...
    abstract class Multi implements Mixed {
      ...
      @Override public final List<List<S2Point>> chains() {
        return new AbstractList<List<S2Point>>() {
          @Override public int size() {
            return numChains();
          }
          @Override public List<S2Point> get(int chainId) {
            int start = vertexId(chainId, edgeId(chainId));
            chainId++;
            int end = chainId == numChains() ? numVertices() : vertexId(chainId, edgeId(chainId));
            return vertices().subList(start, end);
          }
        };
      }
