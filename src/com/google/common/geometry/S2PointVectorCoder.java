...
class S2PointVectorCoder implements S2Coder<List<S2Point>> {
  ...
  private static List<S2Point> decodeCompact(Bytes data, Cursor cursor) {
    ...
    return new AbstractList<S2Point>() {
      ...
      public S2Point get(int index) {
        ...
        int sj = deinterleave1(value);
        int tj = deinterleave2(value);
  ...
  private static ImmutableLongArray convertCellsToValues(List<CellPoint> cellPoints, int level) {
    ...
    for (CellPoint cp : cellPoints) {
      if (cp.level != level) {
        ...
        long v = interleaveBitPairs(sj, tj);
  ...
  /**
   * Like {@link BitInterleave#interleaveInteger(int, int)}, but interleaves bit pairs rather than
   * individual bits. This format is faster to decode than the fully interleaved format, and
   * produces the same results for our use case.
   */
  private static long interleaveBitPairs(int val0, int val1) {
    long v0 = UnsignedInts.toLong(val0);
    long v1 = UnsignedInts.toLong(val1);
    v0 = (v0 | (v0 << 16)) & 0x0000ffff0000ffffL;
    v1 = (v1 | (v1 << 16)) & 0x0000ffff0000ffffL;
    v0 = (v0 | (v0 << 8)) & 0x00ff00ff00ff00ffL;
    v1 = (v1 | (v1 << 8)) & 0x00ff00ff00ff00ffL;
    v0 = (v0 | (v0 << 4)) & 0x0f0f0f0f0f0f0f0fL;
    v1 = (v1 | (v1 << 4)) & 0x0f0f0f0f0f0f0f0fL;
  ...
  /**
   * De-interleaves the first integer from {@code code}.
   *
   * <p>This code is about 50% faster than {@link BitInterleave#deinterleaveInteger(long)}, which
   * uses a lookup table. The speed advantage is expected to be even larger in code that mixes bit
   * interleaving with other significant operations since it doesn't require keeping a 256-byte
   * lookup table in the L1 data cache.
   */
  private static int deinterleave1(long code) {
    long v0 = code;
    v0 &= 0x3333333333333333L;
    v0 |= v0 >>> 2;
    v0 &= 0x0f0f0f0f0f0f0f0fL;
    v0 |= v0 >>> 4;
    v0 &= 0x00ff00ff00ff00ffL;
    v0 |= v0 >>> 8;
    v0 &= 0x0000ffff0000ffffL;
    v0 |= v0 >>> 16;
    return (int) v0;
  }

  /**
   * De-interleaves the second integer from {@code code}.
   *
   * <p>See {@link #deinterleave1(long)}.
   */
  private static int deinterleave2(long code) {
    long v1 = code >>> 2;
    v1 &= 0x3333333333333333L;
    v1 |= v1 >>> 2;
    v1 &= 0x0f0f0f0f0f0f0f0fL;
    v1 |= v1 >>> 4;
    v1 &= 0x00ff00ff00ff00ffL;
    v1 |= v1 >>> 8;
    v1 &= 0x0000ffff0000ffffL;
    v1 |= v1 >>> 16;
    return (int) v1;
  }
}
