  ...
  private void updateFaceEdges(int face, List<FaceEdge> faceEdges, InteriorTracker tracker) {
    ...
    if (numEdges > 0) {
      ...
      if (shrunkId.id() != pcell.id().id()) {
        ...
        skipCellRange(faceId.rangeMin(), shrunkId.rangeMin(), tracker);
        pcell = new S2PaddedCell(shrunkId, CELL_PADDING);
        updateEdges(pcell, clippedEdges, tracker, alloc);
        skipCellRange(shrunkId.rangeMax().next(), faceId.rangeMax().next(), tracker);
        return;
      }
    }
  ...
  private void skipCellRange(S2CellId begin, S2CellId end, InteriorTracker tracker) {
    if (tracker.focusCount > 0) {
      ...
      for (int i = 0; i < skipped.size(); i++) {
        makeLeafCell(new S2PaddedCell(skipped.cellId(i), CELL_PADDING), clippedEdges, tracker);
      }
    }
  }
