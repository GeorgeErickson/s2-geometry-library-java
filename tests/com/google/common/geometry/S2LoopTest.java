...
public strictfp class S2LoopTest extends GeometryTestCase {
  ...
  public void testAreaConsistentWithTurningAngle() throws IllegalAccessException {
    ...
    for (Field field : S2LoopTest.class.getDeclaredFields()) {
      if (S2Loop.class.isAssignableFrom(field.getType())) {
        S2Loop loop = (S2Loop) field.get(this);
        if (loop.isValid()) {
          double area = loop.getArea();
  ...
  public void testEncodeDecodeCompressed() throws IOException {
    ...
    // Snapped loop.
    S2Loop snappedLoopAClone = new S2Loop(snappedLoopA);
  ...
  public void testFourVertexCompressedLoopSize() throws IOException {
    byte[] encoded = encodeCompressed(snappedLoopA, S2CellId.MAX_LEVEL);
  ...
  public void testTurningAngle() {
    ...
    assertEquals(2 * S2.M_PI, lineTriangle.getTurningAngle(), 1e-14);
    checkTurningAngleInvariants(lineTriangle);

    assertEquals(2 * S2.M_PI, skinnyChevron.getTurningAngle(), 1e-14);
  ...
  public void testThreeEdgeShape() {
    assertTrue(smallNeCw.hasInterior());
    assertTrue(smallNeCw.containsOrigin());
    assertEquals(3, smallNeCw.numEdges());
    checkFirstNEdges(smallNeCw, "35:20|45:20, 45:20|40:25, 40:25|35:20");
    assertEquals(1, smallNeCw.numChains());
    checkFirstNChainStarts(smallNeCw, 0);
    checkFirstNChainLengths(smallNeCw, 3);
    checkFirstNChainEdges(smallNeCw, 0, "35:20|45:20, 45:20|40:25, 40:25|35:20");
    assertEquals(2, smallNeCw.dimension());
  }

  public void testFourEdgeShape() {
    assertTrue(eastHemi.hasInterior());
    assertTrue(eastHemi.containsOrigin());
    assertEquals(4, eastHemi.numEdges());
    checkFirstNEdges(eastHemi, "90:0|0:0, 0:0|-90:0, -90:0|0:-180, 0:-180|90:0");
    assertEquals(1, eastHemi.numChains());
    checkFirstNChainStarts(eastHemi, 0);
    checkFirstNChainLengths(eastHemi, 4);
    checkFirstNChainEdges(eastHemi, 0, "90:0|0:0, 0:0|-90:0, -90:0|0:-180, 0:-180|90:0");
    assertEquals(2, eastHemi.dimension());
  }
